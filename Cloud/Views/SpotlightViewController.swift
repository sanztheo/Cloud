...
extension SpotlightViewController: SpotlightTableViewDelegate {
    ...
    func tableView(_ tableView: SpotlightTableView, mouseMovedToEdge edge: SpotlightTableView.Edge) {
        startAutoScroll(for: edge)
    }

    func tableViewMouseMovedAwayFromEdges(_ tableView: SpotlightTableView) {
        stopAutoScroll()
    }

    private func startAutoScroll(for edge: SpotlightTableView.Edge) {
        stopAutoScroll()
        ...
        autoScrollTimer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { [weak self] _ in
            ...
            var newRect = visibleRect

            switch edge {
            case .top:
                newRect.origin.y = max(0, visibleRect.origin.y - 10)
            case .bottom:
                let maxY = self.tableView.bounds.height - visibleRect.height
                newRect.origin.y = min(maxY, visibleRect.origin.y + 10)
            }

            if newRect.origin.y != visibleRect.origin.y {
                self.tableView.scrollToVisible(newRect)
            } else {
                self.stopAutoScroll()
            }
        }
...
class SpotlightTableView: NSTableView {
    ...
    override func mouseMoved(with event: NSEvent) {
        ...
        if point.y > bounds.height - edgeThreshold {
            spotlightDelegate?.tableView(self, mouseMovedToEdge: .top)
        } else if point.y < edgeThreshold {
            spotlightDelegate?.tableView(self, mouseMovedToEdge: .bottom)
        } else {
